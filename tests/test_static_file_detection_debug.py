...
class TestStaticFileDetectionDebug:
    ...
    def test_debug_file_from_log_exact_scenario(self, copy_strategy):
        ...
        print(
            f"Size: {file_from_log.file_size:,} bytes ({file_from_log.file_size / (1024 * 1024):.1f}MB)"
        )
        ...
        print("Expected: False (static file)")
        ...
        print(
            f"1. Growing status check: {status_check} (status: {file_from_log.status})"
        )

        # Check 2: Growth rate
        growth_rate_check = file_from_log.growth_rate_mbps > 0
        print(
            f"2. Growth rate check: {growth_rate_check} (rate: {file_from_log.growth_rate_mbps})"
        )

        # Check 3: Size increase since first seen
        size_increase_check = (
            file_from_log.first_seen_size > 0
            and file_from_log.file_size > file_from_log.first_seen_size
        )
        print(
            f"3. Size increase check: {size_increase_check} (current: {file_from_log.file_size:,}, first: {file_from_log.first_seen_size:,})"
        )

        # Check 4: Recent size change
        recent_change_check = (
            file_from_log.previous_file_size > 0
            and file_from_log.file_size != file_from_log.previous_file_size
        )
        print(
            f"4. Recent change check: {recent_change_check} (current: {file_from_log.file_size:,}, previous: {file_from_log.previous_file_size:,})"
        )

        print("\n=== OVERALL RESULT ===")
        print(
            f"Any condition true: {status_check or growth_rate_check or size_increase_check or recent_change_check}"
        )
        print(f"Should be static: {not result}")

        assert result is False, (
            f"File should be detected as static but was detected as growing. Failing conditions: status={status_check}, rate={growth_rate_check}, size_inc={size_increase_check}, recent={recent_change_check}"
        )
