...
from app.services.destination.destination_checker import (
    DestinationChecker,
...
class TestDestinationCheckerConcurrency:
    ...
    async def test_concurrent_access_protection(self, temp_dest_dir):
        ...
        checker = DestinationChecker(
            temp_dest_dir, cache_ttl_seconds=0.001
        )  # Very short TTL
...
class TestDestinationCheckerEdgeCases:
    ...
    async def test_permission_error_during_check(self):
        ...
        restricted_path = (
            Path("/root")
            if Path("/root").exists()
            else Path("C:\\System Volume Information")
        )
    ...
    async def test_exception_during_write_test(self, temp_dest_dir):
        ...
        with patch(
            "aiofiles.open", side_effect=PermissionError("Mock permission error")
        ):
            result = await checker.test_write_access()
...
class TestDestinationCheckResult:
    ...
    def test_destination_check_result_defaults(self):
        ...
        result = DestinationCheckResult(is_available=False, checked_at=datetime.now())
