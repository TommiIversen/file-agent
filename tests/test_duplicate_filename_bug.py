...
class TestDuplicateFilenameBug:
    ...
    async def test_duplicate_filename_creates_new_tracked_file(
        ...
        await state_manager.update_file_status_by_id(
            first_file.id, FileStatus.COMPLETED
        )
        ...
        test_file_path.write_text(
            "second file content - much longer content to ensure different size"
        )
        ...
        for i, file in enumerate(all_files):
            print(
                f"  File {i + 1}: ID={file.id}, Status={file.status}, Size={file.file_size}, Path={file.file_path}"
            )
        ...
        assert len(completed_files) == 1, (
            f"Expected 1 completed file, got {len(completed_files)}"
        )
        ...
        assert completed_file.id != new_file.id, (
            "Completed and new file should have different IDs"
        )
        assert completed_file.file_size != new_file.file_size, (
            "Files should have different sizes"
        )

        print("✅ SUCCESS: Two separate TrackedFile records created")
        print(
            f"  Completed file: ID={completed_file.id}, Size={completed_file.file_size}"
        )
    ...
    async def test_duplicate_filename_with_multiple_iterations(
        ...
        for iteration in range(3):
            ...
            for file in all_files:
                ...
                ] and file.file_path == str(test_file_path):
                    new_file = file
                    break

            assert new_file is not None, (
                f"New file not found in iteration {iteration + 1}"
            )
            ...
            if iteration < 2:  # Ikke den sidste
                await state_manager.update_file_status_by_id(
                    new_file.id, FileStatus.COMPLETED
                )
        ...
        completed_count = len(
            [f for f in all_files if f.status == FileStatus.COMPLETED]
        )
        active_count = len([f for f in all_files if f.status != FileStatus.COMPLETED])

        assert completed_count == 2, (
            f"Expected 2 completed files, got {completed_count}"
        )
        assert active_count == 1, f"Expected 1 active file, got {active_count}"

        print(
            f"✅ SUCCESS: All {len(all_files)} files have unique IDs and correct status"
        )
        for file in all_files:
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        if len(files) == 1:
            ...
            for i, file in enumerate(files):
                print(
                    f"   File {i + 1}: ID={file.id}, Status={file.status}, Size={file.file_size}"
                )
