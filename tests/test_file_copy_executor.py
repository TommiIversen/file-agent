...
from app.services.copy.file_copy_executor import FileCopyExecutor, CopyResult, CopyProgress
...
class TestFileCopyWithRealFiles:
    ...
    async def test_copy_with_temp_file_success(self, executor, test_file, temp_dir):
        ...
        result = await executor.copy_with_temp_file(test_file, dest_file, progress_callback)
    ...
    @pytest.mark.asyncio
    async def test_verify_copy_failure_missing_dest(self, executor, test_file, temp_dir):
    ...
    @pytest.mark.asyncio
    async def test_verify_copy_failure_size_mismatch(self, executor, test_file, temp_dir):
...
class TestErrorHandlingAndCleanup:
    ...
    async def test_copy_with_temp_file_source_not_found(self, executor):
        ...
        assert result.success is False
        assert "No such file or directory" in result.error_message or "cannot find the file" in result.error_message.lower()
