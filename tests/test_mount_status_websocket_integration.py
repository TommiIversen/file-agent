...
from app.services.storage_monitor.notification_handler import NotificationHandler
...
class TestMountStatusWebSocketIntegration:
    ...
    def create_storage_monitor_with_mocks(self, settings, websocket_manager, network_mount_service):
    ...
    async def test_mount_failure_websocket_broadcast(
        ...
        mock_settings, 
        mock_websocket_manager, 
        mock_network_mount_service_failed
    ):
...
class TestMountStatusSizeCompliance:
    ...
    def test_notification_handler_size_after_mount_integration(self):
        ...
        assert len(code_lines) <= 150, f"NotificationHandler exceeds 150 lines: {len(code_lines)}"
    ...
    def test_websocket_manager_size_after_mount_support(self):
        ...
        assert len(code_lines) <= 400, f"WebSocketManager exceeds 400 lines: {len(code_lines)}"