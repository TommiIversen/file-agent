...
class TestCompletedFilesPersistence:
    ...
    @pytest.mark.asyncio
    async def test_completed_files_survive_cleanup(
        self, 
        state_manager, 
        temp_directory
    ):
    ...
    @pytest.mark.asyncio
    async def test_mixed_files_cleanup_behavior(
        self, 
        state_manager, 
        temp_directory
    ):
    ...
    @pytest.mark.asyncio
    async def test_old_completed_files_cleanup(
        self, 
        state_manager
    ):
        """
    ...
    @pytest.mark.asyncio
    async def test_max_completed_files_limit(
        self, 
        state_manager
    ):
        """
    ...
    @pytest.mark.asyncio
    async def test_completed_files_api_persistence(
        self,
        state_manager,
        temp_directory
    ):
...
async def manual_test_completed_persistence():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        if completed_files:
            print(f"🎉 SUCCESS: Completed file persisted: {completed_files[0].file_path}")
        else:
