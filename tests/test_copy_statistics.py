...
class TestCopySession:
    ...
    def test_copy_session_creation(self):
        ...
        session = CopySession(
            file_path="/test/file.txt", file_size=1000, started_at=start_time
        )
...
class TestStatisticsSummary:
    ...
    def test_success_rate_calculation(self):
        ...
        summary = StatisticsSummary(total_files_copied=8, total_files_failed=2)
...
class TestBasicStatisticsTracking:
    ...
    def test_successful_copy_tracking(self, stats_tracker):
        ...
        stats_tracker.complete_copy_session(
            "/test/file1.txt", success=True, final_bytes_transferred=1000
        )
    ...
    def test_mixed_copy_tracking(self, stats_tracker):
        ...
        stats_tracker.complete_copy_session(
            "/test/file3.txt", success=True, final_bytes_transferred=2000
        )
    ...
    def test_statistics_without_session_tracking(self, stats_tracker_no_sessions):
        ...
        stats_tracker_no_sessions.complete_copy_session(
            "/test/file1.txt", success=True, final_bytes_transferred=1000
        )
        stats_tracker_no_sessions.complete_copy_session(
...
class TestSessionTracking:
    ...
    def test_session_lifecycle(self, stats_tracker):
        ...
        # Complete session
        stats_tracker.complete_copy_session(
            "/test/file.txt", success=True, final_bytes_transferred=1000
        )
    ...
    def test_multiple_concurrent_sessions(self, stats_tracker):
        ...
        stats_tracker.complete_copy_session("/test/file2.txt", success=False)
        stats_tracker.complete_copy_session(
            "/test/file3.txt", success=True, final_bytes_transferred=3000
        )
...
class TestPerformanceMetrics:
    ...
    def test_peak_rate_tracking(self, stats_tracker):
        ...
        with patch("app.services.tracking.copy_statistics.datetime") as mock_dt:
            ...
            mock_dt.now.return_value = start_time
            stats_tracker.start_copy_session("/test/file1.txt", 1000)
            mock_dt.now.return_value = start_time + timedelta(
                seconds=4
            )  # 250 bytes/sec
            stats_tracker.complete_copy_session(
                "/test/file1.txt", success=True, final_bytes_transferred=1000
            ...
            stats_tracker.complete_copy_session(
                "/test/file2.txt", success=True, final_bytes_transferred=2000
            )
...
class TestDetailedStatistics:
    ...
    def test_active_session_details(self, stats_tracker):
        ...
        session1 = next(
            s for s in active_sessions if s["file_path"] == "/test/file1.txt"
        )
        assert session1["progress_percent"] == 50.0
        assert session1["copy_strategy"] == "NormalCopyStrategy"

        session2 = next(
            s for s in active_sessions if s["file_path"] == "/test/file2.txt"
        )
...
class TestMemoryManagement:
    ...
    def test_completed_sessions_limit(self, settings):
        ...
        for i in range(5):
            ...
            tracker.complete_copy_session(
                file_path, success=True, final_bytes_transferred=1000
            )
...
class TestThreadSafety:
    ...
    def test_concurrent_operations(self, stats_tracker):
        ...
        def worker():
            for i in range(10):
                file_path = (
                    f"/test/worker_file_{threading.current_thread().ident}_{i}.txt"
                )
...
class TestEdgeCases:
    ...
    def test_zero_size_files(self, stats_tracker):
        ...
        stats_tracker.complete_copy_session(
            "/test/empty.txt", success=True, final_bytes_transferred=0
        )
