"""Tests for RetryLogic classes — 0 mocks, pure logic."""
...
class TestRetryLimitChecker:
    def test_should_give_up_at_max(self):
        give_up, msg = RetryLimitChecker.should_give_up(5, 5)
        assert give_up is True
        assert "5" in msg

    def test_should_give_up_above_max(self):
        give_up, _ = RetryLimitChecker.should_give_up(10, 5)
        assert give_up is True

    def test_should_not_give_up_below_max(self):
        give_up, msg = RetryLimitChecker.should_give_up(3, 5)
        assert give_up is False
        assert msg == ""

    def test_should_not_give_up_at_zero(self):
        give_up, _ = RetryLimitChecker.should_give_up(0, 5)
        assert give_up is False

    def test_zero_max_always_gives_up(self):
        give_up, _ = RetryLimitChecker.should_give_up(0, 0)
        assert give_up is True
...
class TestRetryDecision:
    def test_proceed_correct_status(self):
        ...
        proceed, _ = RetryDecision.should_retry_proceed(f)
        assert proceed is True

    def test_no_proceed_file_none(self):
        proceed, reason = RetryDecision.should_retry_proceed(None)
    ...
    def test_no_proceed_wrong_status(self):
        ...
        proceed, reason = RetryDecision.should_retry_proceed(f)
        assert proceed is False
    ...
    def test_custom_expected_status(self):
        ...
        proceed, _ = RetryDecision.should_retry_proceed(
            f, expected_status=FileStatus.WAITING_FOR_NETWORK
        )
        assert proceed is True


class TestNetworkRecoveryDecision:
    def test_growing_file_recovers_to_ready_to_start(self):
        f = _make_file(growth_rate_mbps=5.0)
        status, msg = NetworkRecoveryDecision.determine_recovery_status(f)
    ...
    def test_zero_growth_rate_is_static(self):
        f = _make_file(growth_rate_mbps=0.0)
        status, _ = NetworkRecoveryDecision.determine_recovery_status(f)
        assert status == FileStatus.DISCOVERED
