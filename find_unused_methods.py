...
def main():
    ...
    for method in all_methods:
        ...
        external_refs = [
            ref for ref in references if not ref.startswith(method["file"])
        ]

        if len(external_refs) == 0:
            potentially_unused.append({**method, "references": references})
    ...
    for file_path, methods in sorted(by_file.items()):
        ...
        for method in methods:
            ...
            ref_info = (
                f" - {len(method['references'])} local refs"
                if method["references"]
                else " - no refs found"
            )
            print(
                f"  🔸 {method['name']}{class_info} (line {method['line']}){ref_info}"
            )
