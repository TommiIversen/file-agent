...
class WindowsMounter(BaseMounter):
    ...
    async def attempt_mount(self, share_url: str) -> bool:
        ...
        try:
            ...
            self._logger.info(f"Attempting Windows mount: {unc_path}")
            
            if self._drive_letter:
                # Mount to specific drive letter
                cmd = ['net', 'use', f'{self._drive_letter}:', unc_path, '/persistent:yes']
                self._logger.debug(f"Using drive letter {self._drive_letter}: for mount")
            else:
                # Mount without drive letter (UNC access)
                cmd = ['net', 'use', unc_path]
                self._logger.debug("Using UNC path access (no drive letter)")
            ...
            if process.returncode == 0:
                ...
                self._logger.info(f"Successfully mounted {unc_path} as {mount_point}")
                return True
            else:
                error_msg = stderr.decode() if stderr else "Unknown error"
                self._logger.error(f"Mount failed for {unc_path}: {error_msg}")
                return False
                
        except Exception as e:
            self._logger.error(f"Exception during Windows mount attempt: {e}")
            return False
    ...
    def _convert_url_to_unc(self, share_url: str) -> str:
        ...
        try:
            ...
            self._logger.debug(f"Converted {share_url} to UNC path: {unc_path}")
            return unc_path
            
        except Exception as e:
            self._logger.error(f"Error converting URL to UNC path: {e}")
