...
class CopyStatisticsTracker:
    ...
    def complete_copy_session(self, file_path: str, success: bool, final_bytes_transferred: Optional[int] = None) -> None:
        """
        ...
        with self._lock:
            ...
            for session in self._active_sessions.values():
                active_session_details.append({
                    ...
                    "progress_percent": (session.bytes_transferred / session.file_size * 100) if session.file_size > 0 else 0,
                    "duration_seconds": session.duration_seconds,
                    "transfer_rate_mbps": session.transfer_rate_bytes_per_sec / (1024**2),
                    "retry_count": session.retry_count,
                    "copy_strategy": session.copy_strategy
                })
            
            # Get recent completed session summary
            recent_sessions = self._completed_sessions[-self._recent_sessions_window:] if self._completed_sessions else []
            recent_session_summary = {
                "count": len(recent_sessions),
                "average_duration": sum(s.duration_seconds for s in recent_sessions) / len(recent_sessions) if recent_sessions else 0,
                "average_size_mb": sum(s.file_size for s in recent_sessions) / len(recent_sessions) / (1024**2) if recent_sessions else 0,
                "average_rate_mbps": sum(s.transfer_rate_bytes_per_sec for s in recent_sessions) / len(recent_sessions) / (1024**2) if recent_sessions else 0
            }
            ...
            return {
                ...
                "timing": {
                    ...
                    "last_activity_time": summary.last_activity_time.isoformat() if summary.last_activity_time else None
                },
    ...
    def _calculate_current_transfer_rate_mbps(self) -> float:
        ...
        total_rate = sum(session.transfer_rate_bytes_per_sec for session in recent_sessions)
    ...
    def _calculate_average_transfer_rate_mbps(self) -> float:
        ...
        total_bytes = sum(session.bytes_transferred for session in self._completed_sessions)
        total_duration = sum(session.duration_seconds for session in self._completed_sessions)
