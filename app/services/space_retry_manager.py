...
class SpaceRetryManager:
    ...
    async def _schedule_temporary_retry(self, file_path: str, space_check: SpaceCheckResult) -> None:
        """Schedule retry for temporary space shortage (shorter delay)"""
        # Shorter delay for temporary shortages (space might free up soon)
        delay_seconds = self._settings.space_retry_delay_seconds // 2  # Half normal delay
    ...
    async def _schedule_long_retry(self, file_path: str, space_check: SpaceCheckResult) -> None:
    ...
    async def _schedule_retry_task(self, file_path: str, delay_seconds: int, reason: str) -> None:
    ...
    async def _mark_as_permanent_space_error(self, file_path: str, space_check: SpaceCheckResult) -> None:
    ...
    def get_retry_status(self) -> Dict[str, dict]:
        ...
        for file_path, retry_info in self._retry_tracking.items():
            status[file_path] = {
                ...
                "seconds_remaining": max(0, int((retry_info.retry_at - datetime.now()).total_seconds()))
            }
        return status
...
class RetryInfo:
    ...
    def __init__(self, file_path: str, scheduled_at: datetime, retry_at: datetime, reason: str):
