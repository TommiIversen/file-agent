    ...
    async def _process_discovered_files(self, current_files: Set[str]) -> None:
        ...
        for file_path in current_files:
            try:
                ...
                logging.info(
                    f"Ny fil opdaget: {os.path.basename(file_path)} ({file_size} bytes)"
                )
        ...
        try:
            ...
            for tracked_file in all_files_to_check:
                ...
                if self.growing_file_detector:
                    ...
                    await self._handle_growing_file_logic(
                        file_path, current_file_size, current_write_time, tracked_file
                    )
                else:
    ...
    async def _handle_growing_file_logic(
        ...
        try:
            ...
            if tracked_file.status == FileStatus.GROWING:
                logging.info(
                    f"🔄 GROWING FILE CHECK: {os.path.basename(file_path)} "
            ...
            if tracked_file.status in [FileStatus.DISCOVERED, FileStatus.GROWING]:
                (
                    recommended_status,
                    growth_info,
                ) = await self.growing_file_detector.check_file_growth_status(file_path)
                ...
                if recommended_status != tracked_file.status:
                    ...
                    if recommended_status == FileStatus.GROWING:
                        ...
                        logging.info(
                            f"⏳ GROWING FILE STATUS CHANGE: {os.path.basename(file_path)} "
                    ...
                    logging.info(
                        f"Growing file status: {os.path.basename(file_path)} -> {recommended_status.value}"
                    )

            # ALWAYS check for size changes, regardless of status changes above
            if current_file_size != tracked_file.file_size:
                logging.info(
                    f"📏 FILE SIZE CHANGED: {os.path.basename(file_path)} "
                ...
                if tracked_file.status == FileStatus.GROWING:
                    ...
                    logging.info(
                        f"📈 GROWING FILE PROGRESS UPDATE: {os.path.basename(file_path)} "
                ...
                elif tracked_file.status not in [
                    FileStatus.DISCOVERED,
                    FileStatus.GROWING,
                ]:
                    await self.state_manager.update_file_status(
                ...
                logging.debug(
                    f"Growing file size update: {os.path.basename(file_path)} "
        ...
        try:
            ...
            if stable_duration >= self.settings.file_stable_time_seconds:
                ...
                if await self._verify_file_accessible(file_path):
                    ...
                    await self.state_manager.update_file_status(
                        file_path=file_path, status=FileStatus.READY
                    )
                    ...
                    logging.info(
                        f"Fil promoveret til Ready: {os.path.basename(file_path)}"
                    )
                else:
                    logging.warning(
                        f"Fil er stabil men ikke tilgængelig: {os.path.basename(file_path)}"
                    )
