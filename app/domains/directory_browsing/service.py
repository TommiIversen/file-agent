...
class DirectoryScannerService:
    ...
    async def scan_source_directory(
        self, recursive: bool = True, max_depth: int = 3
    ) -> DirectoryScanResult:
    ...
    async def scan_custom_directory(
        self, directory_path: str, recursive: bool = True, max_depth: int = 3
    ) -> DirectoryScanResult:
    ...
    async def _scan_directory(
        ...
        try:
            ...
            logging.info(
                f"Starting {description} directory scan ({scan_mode}): {directory_path}"
            )

            # Wrap the entire scan operation in a timeout
            result = await asyncio.wait_for(
                self._perform_directory_scan(
                    directory_path, recursive=recursive, max_depth=max_depth
                ),
            ...
            scan_duration = (datetime.now() - start_time).total_seconds()
            logging.warning(
                f"{description.title()} directory scan timed out after {scan_duration:.1f}s"
            )
            ...
            scan_duration = (datetime.now() - start_time).total_seconds()
            logging.error(
                f"Error scanning {description} directory {directory_path}: {e}"
            )
    ...
    async def _perform_directory_scan(
        ...
        try:
            path_exists = await asyncio.wait_for(
                aiofiles.os.path.exists(directory_path), timeout=self._item_timeout
            )
            ...
            path_is_dir = await asyncio.wait_for(
                aiofiles.os.path.isdir(directory_path), timeout=self._item_timeout
            )
