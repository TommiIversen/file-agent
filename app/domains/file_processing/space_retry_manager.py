...
from app.domains.file_processing.retry_logic import should_give_up
...
class SpaceRetryManager:
    ...
    async def schedule_space_retry(
        ...
        if space_check.is_temporary_shortage():
            await self._schedule_retry_for_space(tracked_file, space_check, is_temporary=True)
        else:
            await self._schedule_retry_for_space(tracked_file, space_check, is_temporary=False)

    async def _schedule_retry_for_space(
        self, tracked_file: TrackedFile, space_check: SpaceCheckResult, *, is_temporary: bool
    ) -> None:
        if is_temporary:
            delay_seconds = self._settings.space_retry_delay_seconds // 2
            label = "Temporary space shortage"
        else:
        ...
        try:
            await self._state_machine.transition(
                ...
                error_message=f"{label}: {space_check.reason}. Retrying in {delay_seconds // 60} minutes."
            )
        except (InvalidTransitionError, ValueError) as e:
            logging.warning(f"Could not set file {tracked_file.id} to WAITING_FOR_SPACE: {e}")
            return

        success = await self.schedule_retry(
            tracked_file.id,
            delay_seconds,
            f"{label}: {space_check.reason}",
            "space",
        )

        retry_type = "SHORT" if is_temporary else "LONG"
        if success:
            logging.debug(
                f"SPACE RETRY {retry_type}: {tracked_file.file_path} [UUID: {tracked_file.id[:8]}...]"
            )
        else:
            logging.warning(
                f"Failed to schedule {retry_type.lower()} retry for {tracked_file.file_path}"
            )
    ...
    async def _mark_as_permanent_space_error(
        ...
        try:
            await self._state_machine.transition(
                ...
                space_error_at=datetime.now(timezone.utc),
            ...
            logging.warning(f"Could not set file {tracked_file.id} to SPACE_ERROR: {e}")
