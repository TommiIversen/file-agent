            ...
            if isinstance(self.resumable_strategy, ResumeCapableMixin):
                ...
                if await self.resumable_strategy.should_attempt_resume(source, dest):
                    ...
                    if success:
                        ...
                        logger.warning(f"Resume failed - falling back to fresh copy: {source.name}")
                
                # Fallback til fresh copy
                logger.debug(f"Udfører fresh copy: {source.name}")
                return await self.resumable_strategy.execute_copy(source, dest, progress_callback)
                ...
                if hasattr(self.resumable_strategy, 'execute_copy'):
                    ...
                    return await self.resumable_strategy.copy_file(source_path, dest_path, tracked_file)
...
def get_resume_config_for_mode(mode: str):
    ...
    """
    from ..utils.secure_resume_config import CONSERVATIVE_CONFIG, PERFORMANCE_CONFIG, PARANOID_CONFIG
...
class ResumeMetricsCollector:
    ...
    def get_average_preservation(self) -> float:
        ...
        return sum(op.preservation_percentage for op in self.operations) / len(self.operations)
    ...
    def log_summary(self):
        ...
        logger.info(f"  Corruption detected: {corrupted}/{len(self.operations)} ({(corrupted/len(self.operations))*100:.1f}%)")
