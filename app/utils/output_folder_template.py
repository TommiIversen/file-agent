...
class OutputFolderTemplateEngine:
    ...
    def __init__(self, settings: Settings):
        ...
        self.logger.info(f"OutputFolderTemplateEngine initialized with {len(self.rules)} rules")
        
        if self.rules:
            for rule in self.rules:
                self.logger.debug(f"Rule: pattern='{rule.pattern}' → folder='{rule.folder_template}'")
    ...
    def generate_output_path(self, filename: str, source_path: str = "") -> str:
        ...
        if matching_rule:
            ...
            self.logger.debug(f"Using default category for '{filename}': {folder_template}")
        ...
        output_path = Path(self.settings.destination_directory) / output_subfolder / filename
    ...
    def get_output_subfolder(self, filename: str) -> str:
        ...
        matching_rule = self._find_matching_rule(filename)
        folder_template = matching_rule.folder_template if matching_rule else f"{self.default_category}\\{{date}}"
    ...
    def _parse_template_rules(self) -> List[TemplateRule]:
        ...
        try:
            ...
            if self.settings.output_folder_rules.strip().startswith('['):
                ...
                rule_strings = [r.strip() for r in self.settings.output_folder_rules.split(',')]
