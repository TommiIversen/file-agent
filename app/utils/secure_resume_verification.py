...
class SecureVerificationEngine:
    ...
    async def find_safe_resume_position(
        ...
        try:
            ...
            if self.config.detailed_corruption_logging:
                logger.info(f"Starter verification: source={source_size:,}B, dest={dest_size:,}B")
            ...
            if initial_verification_ok:
                ...
                if self.config.detailed_corruption_logging:
                    logger.info(f"Verification SUCCESS: resuming fra {dest_size:,} bytes")
                
                return dest_size, success_metrics
            
            # Corruption detekteret - find præcist hvor
            logger.warning("Corruption detekteret i verification region - starter binary search")
            
            safe_position, binary_iterations = await self._binary_search_corruption_point(
    ...
    async def _verify_region_exact(
        ...
        try:
            async with self._open_files_for_verification(source_path, dest_path) as (src_file, dst_file):
                ...
                while remaining > 0:
                    ...
                    if self.config.log_verification_progress and bytes_verified % (10 * 1024 * 1024) == 0:
    ...
    async def _binary_search_corruption_point(
        ...
        while right - left > chunk_size and iterations < max_iterations:
            ...
            mid = left + ((right - left) // chunk_size) * chunk_size  # Align til chunk boundary
            ...
            try:
                ...
                if region_ok:
                    ...
                    if self.config.log_verification_progress:
                        logger.debug(f"Region OK - corruption efter {last_good_position:,}")
                else:
...
class QuickIntegrityChecker:
    ...
    @staticmethod
    async def quick_tail_check(source_path: Path, dest_path: Path, check_bytes: int = 1024) -> bool:
        """
