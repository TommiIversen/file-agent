...
class SecureResumeConfig(BaseModel):
    ...
    max_verify_mb: int = Field(
        ...
        ge=1, le=1000  # Mellem 1MB og 1GB
    )
    ...
    verification_buffer_kb: int = Field(
        ...
        ge=4, le=1024  # Mellem 4KB og 1MB
    )
    ...
    detailed_corruption_logging: bool = Field(
        default=True,
        description="Aktiver detaljeret logging af corruption detection"
    )
    ...
    def log_config(self):
        ...
        logger.info(f"  Verification: {self.min_verify_bytes} bytes - {self.max_verify_mb} MB")
        logger.info(f"  Binary search: {self.binary_search_chunk_mb} MB chunks")
        logger.info(f"  Buffer: {self.verification_buffer_kb} KB")
        logger.info(f"  Store filer: >{self.large_file_threshold_mb} MB → {self.large_file_verification_mb} MB verification")
...
class ResumeOperationMetrics(BaseModel):
    ...
    def log_metrics(self):
        ...
        logger.info(f"  Verificeret: {self.verification_bytes:,} bytes ({self.verification_percentage:.1f}%)")
        ...
        logger.info(f"  Bytes bevaret: {self.bytes_preserved:,} ({self.preservation_percentage:.1f}%)")
