#!/bin/bash
# File Transfer Agent - Comprehensive macOS Uninstall Script
...
uninstall_browser_agent() {
    ...
    # Unload agent
    if launchctl list | grep -q "${BROWSER_PLIST_NAME%.*}"; then # Use %.* to remove .plist extension for grep
...
main() {
    ...
    log_success "✅ Comprehensive uninstall complete. Virtual environment and project files were not removed."
}
